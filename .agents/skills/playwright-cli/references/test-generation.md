...
Plan / generate / heal lean on the same mechanic: run `pnpm run test:e2e --debug=cli` in the background, then `pnpm exec playwright-cli attach tw-XXXX` to drive the paused page interactively. See [playwright-tests.md](playwright-tests.md) for the debug/attach mechanics.
...
If Playwright is missing, stop and report the broken project installation; do not fetch a remote scaffolder.
...
```bash
PLAYWRIGHT_HTML_OPEN=never pnpm run test:e2e tests/seed.spec.ts --debug=cli
...
```bash
PLAYWRIGHT_HTML_OPEN=never pnpm run test:e2e <seed-file> --debug=cli   # background
pnpm exec playwright-cli attach tw-XXXX
# resume
```
...
```bash
PLAYWRIGHT_HTML_OPEN=never pnpm run test:e2e tests/<group>/<scenario>.spec.ts
```
...
```bash
PLAYWRIGHT_HTML_OPEN=never pnpm run test:e2e
```
...
```bash
PLAYWRIGHT_HTML_OPEN=never pnpm run test:e2e tests/<group>/<scenario>.spec.ts:<line> --debug=cli
