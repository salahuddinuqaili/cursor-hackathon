...
```bash
pnpm exec shadcn mcp        # start the MCP server (stdio)
pnpm exec shadcn mcp init   # write config for your editor
```
...
> **Tip:** MCP tools handle registry operations (search, view, install). For project configuration (aliases, framework, Tailwind version), use `pnpm exec shadcn info` — there is no MCP equivalent.
