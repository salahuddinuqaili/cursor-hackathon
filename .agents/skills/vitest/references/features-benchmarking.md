...
```bash
pnpm exec vitest bench           # only benchmarks (implicitly enables them)
pnpm exec vitest bench parser    # filter by filename
pnpm exec vitest bench -t JSON   # filter by test name
```
