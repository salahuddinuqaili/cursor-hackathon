...
# Machine 3
pnpm exec vitest run --shard=3/3
```
...
jobs:
  ...
  merge:
    needs: test
    steps:
      - run: pnpm exec vitest --merge-reports --reporter=junit
```
...
pnpm exec vitest run --shard=1/3 --reporter=blob --coverage
pnpm exec vitest run --shard=2/3 --reporter=blob --coverage

# Merge all blobs
pnpm exec vitest --merge-reports --reporter=json --coverage
```
...
```bash
pnpm exec vitest --bail 1    # Stop after 1 failure
pnpm exec vitest --bail      # Stop on first failure (same as --bail 1)
```
