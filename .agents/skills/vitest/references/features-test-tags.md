...
```bash
pnpm exec vitest --tagsFilter "frontend"
pnpm exec vitest --tagsFilter "db && !flaky"
pnpm exec vitest --tagsFilter "(unit || e2e) && !slow"
pnpm exec vitest --tagsFilter "api/*"            # wildcard
pnpm exec vitest --list-tags                     # list defined tags (=json for JSON)
```
